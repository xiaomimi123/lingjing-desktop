...
async function handleSend() {
  ...
  // 立刻清空输入框 —— 不要等 RPC 返回。OpenClaw Win 上 chat.send RPC
  // 可能 5-20 秒才回,如果在 await 之后才清 draft 和滚动,user 视觉上感觉
  // "消息被隐藏 20 秒,等 AI 回复出来才看到自己消息"。
  draft.value = ''
  ...
  try {
    ...
    // sendMessage 内部同步 push 乐观消息后才 await rpc。我们不等整个 promise,
    // 而是在 push 完成的下一个 tick 立即滚动到底部,让 user 立刻看到自己气泡。
    const sendPromise = chatStore.sendMessage(content)
    await nextTick()
    autoFollowBottom.value = true
    requestScrollToBottom({ force: true })
    ...
    // 失败时把 draft 恢复,避免 user 丢内容(因为我们提前清空了输入框)
