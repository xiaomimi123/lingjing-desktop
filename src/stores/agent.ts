...
export const useAgentStore = defineStore("agent", () => {
  ...
  async function addAgent(params: {
    ...
    // 新 agent 在 OpenClaw 内部默认无 model 配置,导致发消息时 Gateway
    // 不知道用哪个 LLM,前端表现为"60s 不响应"。主动从 main agent 继承 model,
    // 让用户创建后可立刻发消息。失败不阻塞,沿用旧行为。
    try {
      await fetchAgents();
      const mainAgent = agents.value.find((a) => a.id === "main");
      const newAgent = agents.value.find((a) => a.id === agentId);
      if (mainAgent?.model && !newAgent?.model) {
        console.log(
          `[AgentStore] 复制 main 的 model 给新 agent ${agentId}: ${mainAgent.model}`,
        );
      ...
      console.warn("[AgentStore] 自动从 main 继承 model 失败,不阻塞创建:", e);
    }
