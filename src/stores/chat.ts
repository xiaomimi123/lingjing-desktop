...
export const useChatStore = defineStore('chat', () => {
  ...
  async function fetchHistory(
    ...
    try {
      ...
      const serverHistory = await wsStore.rpc.listChatHistory(normalizedKey)
      // 只在 sending=true(真正还有 in-flight RPC)时保留本地乐观消息;
      // sending 完成后 server 已收录(用 server-id 而非 web-id),直接替换。
      // 不能用 id 严格匹配判断"已收录"——server-id 跟乐观 web-id 永远不同,
      // 否则乐观消息会被永久残留,UI 上越来越多幽灵消息追加在末尾。
      if (sending.value) {
        const serverIds = new Set(serverHistory.map((m) => m.id))
        const inflightLocal = messages.value.filter(
          (m) => m.role === 'user' && typeof m.id === 'string' && m.id.startsWith('web-') && !serverIds.has(m.id),
        )
        messages.value = inflightLocal.length > 0 ? [...serverHistory, ...inflightLocal] : serverHistory
      } else {
