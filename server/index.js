...
function readOpenClawAuthToken() {
  try {
    ...
    console.warn('[OpenClaw] 读 ~/.openclaw/openclaw.json 失败,使用 .env token:', e.message)
    return null
  }
}

const openclawTokenFromConfig = readOpenClawAuthToken()
const effectiveOpenClawToken = openclawTokenFromConfig || envConfig.OPENCLAW_AUTH_TOKEN
if (openclawTokenFromConfig) {
  console.log(`[OpenClaw] 使用 ~/.openclaw/openclaw.json 的 gateway.auth.token (suffix: ...${openclawTokenFromConfig.slice(-6)})`)
} else if (envConfig.OPENCLAW_AUTH_TOKEN) {
  console.log('[OpenClaw] 使用 .env OPENCLAW_AUTH_TOKEN (fallback)')
} else {
  console.warn('[OpenClaw] 未找到 token! 请运行 openclaw onboard 或在 .env 设 OPENCLAW_AUTH_TOKEN')
}
...
let gateway = new OpenClawGateway(envConfig.OPENCLAW_WS_URL, effectiveOpenClawToken, envConfig.OPENCLAW_AUTH_PASSWORD, envConfig.LOG_LEVEL)
